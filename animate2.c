...
int main(void)
{
	...
	int divx = 2 * rad; int divy = 100; // レイヤーのずれの値
	...
	for (t = 0.0; t <= 10*PI(); t += PI()/10)
	{
		...
		img_drawcycloid3(green, rad, 0 +divx, 100-rad +divy, t * 180 / PI());

		img_drawcycloidpoint2(red, rad, 0 +divx, 100-rad +divy, t * 180 / PI(), rad/3);
		img_drawtrochoid2(white, 0 +divx, 100-rad +divy, rad, rad*2, t * 180 / PI());

		img_drawtrochoidpoint2(darkred, 0 +divx, 100-rad +divy, rad, rad*2, t * 180 / PI(), rad/6);

		img_drawtrochoid2(pink, 0 +divx, 100-rad +divy, rad, rad/2, t * 180 / PI());
		img_drawtrochoidpoint2(darkred, 0 +divx, 100-rad +divy, rad, rad/2, t * 180 / PI(), rad/6);
